...
class TestMessageAutoSubscribeNotifyOwn(TransactionCase):
    def setUp(self):
        ...
        self.env["auto.subscribe.notify.own.model"].create(
            dict(model_id=self.fake_model_id.id)
        )
        self.fake_test = self.env["message_auto_subscribe_notify_own.test"].create(
            dict(name="Test Fake Model", user_id=self.env.ref("base.user_demo").id)
        )
    ...
    def test_message_auto_subscribe_notify_own(self):
        ...
        self.fake_test.update(dict(user_id=self.user_root.id))
    ...
    def test_compute_name(self):
        ...
        self.assertTrue(notified_model.name, "message_auto_subscribe_notify_own.test")
